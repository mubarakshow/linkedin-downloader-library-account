...
const LinkedInLearningDownloader = () => {
  ...
  async function login(user, password) {
    try {
      const loginPageURL = `https://www.linkedin.com/learning-login/go/account/${process.env.ACCOUNT_ID}`;
    
    // Library Login
      await page.goto(loginPageURL);
      await Promise.all([
        page.click(".library-go__anchor--desktop"),
        page.waitForNavigation(),
      ]);
      await page.type("#library-card-number-input", user);
      await page.type("#library-card-pin-input", password);
      await Promise.all([
  ...
  function getItemsToDownloadFromList(itemsURLs) {
    ...
    const items = itemsURLs.map((url) =>
      url.replace(/.*\/learning\/([^?]*).*/, "$1")
    );
    return {
      ...
      individualCourses: items
        .filter((e) => !e.match(/^(me\/|paths\/|collections\/)/))
        .map((e) => e.replace(/([^\/]*).*/, "$1")),
    };
  }
  ...
  async function getAllSavedCourses() {
    ...
    return await page.$$eval(
      'a.entity-link[data-control-name="card_title"]',
  ...
  async function getCoursesFromCollection(collection) {
    ...
    await scrollToBottom(page);
    return await page.$$eval(
      'a.entity-link__link[data-control-name="collection_card"]',
      (a) => a.map((e) => e.href.replace(/.*\/learning\/([^\/?]*).*/, "$1"))
    );
  }
  ...
  async function getAllInProgressCourses() {
    ...
    const collapsedPaths = await page.$$(
      ".lls-card-child-content__button>span"
    );
    ...
    return await page.$$eval(
      'a.card-entity-link[data-control-name="card_title"]',
      (a) =>
        a
          .map((e) => e.href.replace(/.*\/learning\/([^\/?]*).*/, "$1"))
          .filter((e) => e != "paths")
    );
  }
  ...
  async function getPathStructure(path) {
    ...
    const title = await page.$eval(".path-layout__header-main h1", (h1) =>
      h1.textContent.trim()
    );
    const startButton = await page.$(
      'button[data-control-name="start_learning_path"]'
    );
    if (startButton) {
    ...
    const courseURLs = await page.$$eval(
      'a.entity-link__link[data-control-name="path_card_title"]',
      (l) => l.map((a) => a.href.replace(/.*\/learning\/([^\/?]*).*/, "$1"))
    );

    const courseTitles = await page.$$eval(".lls-card-headline", (l) =>
      l.map((span) => span.textContent.trim())
    );

    return {
      title,
      courses: courseURLs.map((courseURL, i) => ({
        url: courseURL,
  ...
  async function getCourseStructure(course) {
    try {
      ...
      const courseTitle = makeFileSystemSafe(
        decodeHTML(
          await page.$eval(".classroom-nav__details h1", (el) =>
            el.textContent.trim()
          )
        )
      );
      ...
      const collapsedChapters = await page.$$(
        ".classroom-toc-chapter--collapsed"
      );
      ...
      const HTMLStructure = await page.evaluate(() =>
        [...document.querySelectorAll(".classroom-toc-chapter")].map(
          (chapter, chapterId) => ({
            title: chapter.querySelector(".classroom-toc-chapter__toggle-title")
              .innerHTML,
            lessons: [
              ...chapter.querySelectorAll(".classroom-toc-item__link"),
            ].map((lesson) => ({
              url: lesson.href,
              title: lesson.querySelector(".classroom-toc-item__title")
      ...
      const chapters = HTMLStructure.map((chapter, chapterId) => ({
        title: makeFileSystemSafe(
          decodeHTML(
            chapter.title
              .trim()
      ...
      console.error(
        `Unexpected error while fetching chapter list of course ${course} : ${err}`
      );
      return null;
    }
  }
    ...
    try {
      ...
      for (uri = null, tryCount = 0; uri === null && tryCount < 5; tryCount++) {
        ...
        if (uri == null && tryCount < 5) {
          console.warn(
            `Trouble fetching video uri for '${lesson.title}' : retrying`
          );
          await timeout(4000);
        }
      }
      if (uri == null) {
        console.warn(
          `Could not fetch video uri for ${lesson.title} : skipped !`
        );
        retryCount = 0;
        return false;
      }
      ...
      try {
        await Promise.race([
          ...
          timeout(3 * 60 * 1000).then(() => {
            throw new Error("Download timeout");
          }), // retry downloading after 3 minutes
        ]);
      } catch (err) {
      ...
      console.info(
        ` '${output.replace(/.*\/[^\/]*\/([^\/]*\/[^\/]*)/, "$1")}' downloaded.`
      );
      return true;
    } catch (err) {
      console.error(
        `Unexpected error while downloading lesson '${lesson.title}' : ${err}`
      );
      return false;
    }
  }
  ...
  async function downloadCourses({
    ...
    try {
      ...
      const {
        ...
        paths,
        collections, // specific public learning paths and personal collections of courses
        allSaved,
        allCompleted,
        allInProgress, // whether to include personal My Learning saved/completed/in progress courses
      ...
      const allCourses = [
        ...
        ...(await collections.reduce(
          async (l, c) => [
            ...(await l),
            ...(await getCoursesFromCollection(c)),
          ],
          Promise.resolve([])
        )),
      ...
      for (const course of distinctCourses) {
        ...
        do {
          ...
          if (structure === null && retryCount < 5) {
            console.warn(
              `Trouble getting course structure for '${course}' : retrying`
            );
            timeout(2000);
          }
        ...
        if (structure === null) {
          console.warn(
            `<!!!> Could not get course structure for '${course}' : skipped !`
          );
          skipCount++;
          continue;
        }
        ...
        for (const chapterId in structure.chapters) {
          ...
          for (
            ...
            const lessonFullPath = `${chapterPath}/${lessonId + 1}. ${
              lesson.title
            }.mp4`;
            if (
              fs.existsSync(lessonFullPath) &&
              fs.statSync(lessonFullPath).size > 200000
            ) {
              continue;
            }
            // Try download lesson
            ...
            do {
              ...
              if (!ok && retryCount < 5) {
                console.warn(
                  `Trouble downloading video for '${lesson.title}' : retrying`
                );
                timeout(2000);
              }
            ...
            if (!ok) {
              console.warn(
                `<!!!> Could not download video for ${lesson.title} : skipped !`
              );
              skipCount++;
              continue;
            }
